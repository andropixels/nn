...
import { createInstruction, deleteInstruction, deleteNameRegistry, getFilteredProgramAccounts, getHashedName, getNameAccountKey, NameRegistryState, NAME_PROGRAM_ID, Numberu32,
   Numberu64, transferInstruction, updateInstruction } from '@solana/spl-name-service';
import {
    Connection,
    PublicKey,
    SystemProgram,
  ...
  export async function getTwitterRegistryKey(
    ...
    const hashedTwitterHandle = await getHashedName(twitter_handle);
       console.log(`${hashedTwitterHandle} this  is one lers see `);
       
    let f = await getNameAccountKey(
      hashedTwitterHandle,
      undefined,
      TWITTER_ROOT_PARENT_REGISTRY_KEY
    );
    console.log(`${f} this  is one lers see `);
    return f ;
  }
  ...
  export async function getTwitterRegistry(
    ...
    //   TWITTER_ROOT_PARENT_REGISTRY_KEY
    // );
    const twitterHandleRegistryKey=  await getTwitterRegistryKey(twitter_handle  );

    console.log(`${twitterHandleRegistryKey} thvgghgg`);
